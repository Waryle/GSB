...
class connexionPDO
{
    ...
    {
        $req      = "SELECT  REG_CODE, TRA_ROLE FROM travailler where  VIS_MATRICULE = '$idcolaborateur'";
        $res      = connexionPDO::$monPdo->query($req);
        $rep      = $res->fetch();
    ...
    {
        $req = "SELECT  MED_DEPOTLEGAL, MED_NOMCOMMERCIAL,FAM_CODE, MED_COMPOSITION, MED_EFFETS, MED_CONTREINDIC, MED_PRIXECHANTILLON FROM medicament where MED_DEPOTLEGAL ='$medicament' ";
        $res = connexionPDO::$monPdo->query($req);
        $rep = $res->fetch();
        return $rep;
